...
* Uses HAML as an HTML abstraction layer that can be compiled out to various doctypes allowing me to focus more 
  ...
  tag
* Uses LESS for CSS to allow the use of Less's syntax for variables and mixins. (HAML comes packaged with the 
  SASS system for CSS that offers similar options, but I just wasn't able to adjust to the syntax.)
